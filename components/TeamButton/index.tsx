...
const TeamButton: React.FC<TeamButtonProps> = ({ label, className = "", style = {} }) => {

  const containerStyles: CSSProperties = {
    display: 'inline-block',
  };

  const buttonClasses = `font-bold py-2 px-4 gap-[8px] rounded inline-flex items-center ButtonContainer ${className}`;

  return (
    <div style={containerStyles}>
      <button className={buttonClasses} style={style}>
        <span style={{ fontSize: '13px' }}>About {label}</span>
